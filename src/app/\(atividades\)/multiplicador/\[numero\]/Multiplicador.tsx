...
function DigitInput(props: {
  ...
  return (
    <div className="flex flex-col items-center gap-1 w-full border-2 border-[var(--color-border)] rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-center gap-1 w-full ">
        ...
        <input
          ...
          pattern="[0-9]*"
          maxLength={1}
          className="cursor-text h-7 sm:h-11 md:h-14 w-7 sm:w-8 md:w-9 my-1 text-center text-xs sm:text-lg md:text-2xl font-extrabold border-b-3 bg-[var(--color-surface)] text-[var(--color-text-primary)] hover:bg-gray-100 focus-within:border-[var(--color-primary)] focus:outline-none"
...
export default function Multiplicador({ dadosMult }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 sm:gap-6 relative">
      ...
      <div className="w-full border border-[var(--color-border)] rounded-xl overflow-hidden">
        <div className="flex justify-between items-center px-4 py-2 bg-[var(--color-surface)]">
          <span className="font-bold text-sm sm:text-base">🎥 Vídeo Explicativo</span>
          <button onClick={() => setVideoOpen(!videoOpen)} className="text-sm font-bold cursor-pointer">{videoOpen ? "-" : "+"}</button>
        </div>
        {videoOpen && (
          <div className="w-full bg-black p-2">
            <YouTube videoId={dadosMult.videoUrl} opts={{ width: "100%", height: typeof window !== 'undefined' && window.innerWidth < 640 ? "150" : "360", playerVars: { autoplay: 1 } }} iframeClassName="w-full aspect-video max-h-[150px] sm:max-h-[360px]" />
          </div>
        )}
      </div>
      ...
      <div className="w-full flex flex-col gap-1 sm:gap-2">
        ...
        <div className="relative w-full">
          ...
          <div className="w-full overflow-x-auto p-1 sm:p-3 md:p-4 overflow-y-visible">
            <div ref={rowRef} className="relative grid grid-cols-[repeat(5,minmax(50px,1fr))_auto] sm:grid-cols-[repeat(5,minmax(60px,1fr))_auto] gap-1 items-center justify-center min-w-max">

            {/* O dígito 0 é fixo à esquerda (o vizinho fantasma do Trachtenberg) */}
            <select disabled value={selects[0]} className="h-7 sm:h-10 md:h-11 rounded-lg text-center font-bold text-xs sm:text-sm bg-[var(--color-border)] opacity-60"><option value="0">0</option></select>
            ...
            {[1, 2, 3, 4].map((idx) => (
              <select
                ...
                className="h-7 sm:h-10 md:h-11 rounded-lg text-center font-bold text-xs sm:text-sm bg-[var(--color-primary)] text-white border border-[var(--color-border)]"
              >
                <option value="">-</option>
      ...
      <div className="w-full border border-[var(--color-border)] rounded-xl overflow-hidden">
        <div className="flex justify-between items-center px-4 py-2 bg-[var(--color-surface)]">
          <span className="font-bold text-sm sm:text-base">Passo a passo</span>
          <button onClick={() => setHelpOpen(!helpOpen)} className="text-sm font-bold cursor-pointer">{helpOpen ? "-" : "+"}</button>
        </div>
        {helpOpen && (
          <div className="flex flex-col border-t border-[var(--color-border)]">
            <div className="flex bg-[var(--color-surface)]">
              {[5, 4, 3, 2, 1].map((d) => (
                <button key={d} onClick={() => setHelpDigit(d as DigitIndex)} className={`flex-1 py-1 text-xs sm:text-sm font-bold border-r border-[var(--color-border)] ${helpDigit === d ? "bg-[var(--color-primary)] text-white" : ""}`}>{d}º</button>
              ))}
            </div>
            <div className="p-2 sm:p-4 min-h-[60px] sm:min-h-[100px] text-xs sm:text-sm">{helpDigit ? dadosMult.helpText[helpDigit] : "Selecione um dígito para ajuda."}</div>
          </div>
        )}
      </div>

      {/* Botões */}
      <div className="flex flex-row gap-1 sm:gap-3 w-full">
        <button onClick={conferir} className="flex-1 rounded-lg sm:rounded-xl py-2 sm:py-3 text-xs sm:text-base font-bold bg-[var(--color-primary)] text-white transition-colors hover:bg-[var(--color-primary-hover)]">Conferir</button>
        <button onClick={limpar} className="flex-1 rounded-lg sm:rounded-xl py-2 sm:py-3 text-xs sm:text-base font-bold border border-[var(--color-border)] hover:bg-gray-50">Limpar</button>
        <button onClick={pegarNumAleatorio} className="flex-1 rounded-lg sm:rounded-xl py-2 sm:py-3 text-xs sm:text-base font-bold border border-[var(--color-border)] hover:bg-gray-50">Aleatório</button>
      </div>
    </div>
  );
}