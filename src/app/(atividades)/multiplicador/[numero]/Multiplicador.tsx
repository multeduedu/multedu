...
export default function Multiplicador({ dadosMult }: Props) {
  ...
  function conferir() {
    ...
    if (a === b) {
      ...
      addCoins(10).catch((err) => console.error("Erro ao adicionar XP:", err));
      Swal.fire({
        ...swalBase,
        title: "✅ Acertou!",
  ...
  const onVideoError: YouTubeProps["onError"] = () => {
    Swal.fire({
      ...swalBase,
      title: "⚠️ Erro ao carregar vídeo",
      text: "Não foi possível carregar o vídeo. Tente novamente.",
      icon: "warning",
      confirmButtonText: "Ok",
    });
  };
  ...
  return (
    <div className="flex flex-col items-center gap-6 relative">
      ...
      <div className="w-full border border-[var(--color-border)] rounded-xl overflow-hidden">
        <div className="flex justify-between items-center px-4 py-2 bg-[var(--color-surface)]">
          ...
          <button
            ...
            className="text-sm font-bold cursor-pointer hover:text-[var(--color-primary)] transition-colors"
            aria-label="Expandir ou recolher seção de vídeo"
          >
            {videoOpen ? "-" : "+"}
          </button>
        </div>
      ...
      <div className="w-full">
        <h3 className="text-base mb-4 font-semibold text-[var(--color-text-primary)]">
          ...
          <fieldset>
            ...
            >
              <div
                ref={arrowRef}
                aria-hidden="true"
                className="pointer-events-none absolute top-0 transition-opacity duration-300 opacity-0 -translate-x-1/2 -translate-y-full"
              >
              ...
              <select
                ...
                aria-label="Primeiro dígito (casa das dezenas de milhar) - bloqueado em 0"
                className="cursor-not-allowed h-8 sm:h-11 rounded-lg text-center font-bold text-sm sm:text-lg
               bg-[var(--color-border)] text-[var(--color-text-secondary)] border border-[var(--color-border)] opacity-60"
              >
                <option value="0">0</option>
              </select>

              {[
                { id: "select4", idx: 1, bg: "bg-[var(--color-primary)] text-white" },
                { id: "select3", idx: 2, bg: "bg-[var(--color-primary)] text-white" },
                { id: "select2", idx: 3, bg: "bg-[var(--color-primary)] text-white" },
                { id: "select1", idx: 4, bg: "bg-[var(--color-primary)] text-white" },
              ].map((s) => (
                <select
                  key={s.id}
                  ref={(el) => {
                  ...
                  aria-label={`Selecione o ${["quinto", "quarto", "terceiro", "segundo"][s.idx - 1]} dígito (multiplicando)`}
      ...
      <div className="w-full border border-[var(--color-border)] rounded-xl overflow-hidden">
        <div className="flex justify-between items-center px-4 py-2 bg-[var(--color-surface)]">
          ...
          <button
            ...
            className="text-sm font-bold cursor-pointer hover:text-[var(--color-primary)] transition-colors"
            aria-label="Expandir ou recolher seção de ajuda"
          >
            {helpOpen ? "-" : "+"}
          </button>
        </div>
      ...
      {/* Info */}
      <div className="w-full text-center text-sm text-[var(--color-text-secondary)]">
        <span className="font-semibold">Número atual:</span>{" "}
        <span className="font-mono text-base">{getNumeroOriginal()}</span>
      </div>
    </div>
  );
}
