...
export default function MultiplicadorX4() {
  ...
  const HELP_TEXT: Record<DigitIndex, string> = {
    1: 'Subtraímos por 10 e se impar adicionar +5',
    2: 'Subtrair por 9 e adicionar a metade do vizinho, se impar adicionar +5',
    3: 'Subtrair por 9 e adicionar a metade do vizinho, se impar adicionar +5',
    4: 'Subtrair por 9 e adicionar a metade do vizinho, se impar adicionar +5',
    5: 'Pegar a metade do vizinho e subtrair 1',
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const correctAnswer = String(original * 4)
      ...
      if (answerNormalized === correctNormalized) {
        ...
        Swal.fire({
          title: '✅ Acertou!',
          html: `A multiplicação de <strong>${displayOriginal} × 4</strong> é:<br><strong>${correctAnswer}</strong>`,
          icon: 'success',
        ...
        Swal.fire({
          title: '❌ Errou!',
          html: `Sua resposta <strong>(${answer})</strong> está incorreta.<br>O resultado correto de <strong>${displayOriginal} × 4</strong> é: <strong>${correctAnswer}</strong>`,
          icon: 'error',
          confirmButtonText: 'Entendi!',
          customClass: {
            container: 'swal2-custom',
  ...
  return (
    <div className="flex flex-col items-center">
      <h2 className="sr-only">Treino de multiplicação por 4</h2>
      ...
      <div className="w-full">
          ...
          <div className="mx-auto flex flex-nowrap justify-center gap-0.5 sm:gap-4 min-w-max px-2 py-1">
            {[5, 4, 3, 2, 1].map((digit) => (
              <button
                key={digit}
                type="button"
                onClick={() => showHelp(digit as DigitIndex)}
        ...
        <div className="w-full overflow-x-auto overflow-y-hidden [-webkit-overflow-scrolling:touch] mb-6">
          ...
          {[1, 2, 3, 4, 5].map((digit) => (
            <select
              ...
              disabled={digit === 5}
              className={`h-8 sm:h-11 w-[42px] sm:w-[76px] rounded-lg font-bold text-center border-2 transition-colors ${
                digit === 5
                  ...
                  : digit === 4
                    ...
                    : digit === 3
                      ...
                      : digit === 2
                        ...
                        : 'bg-cyan-500 text-gray-900 border-cyan-600'
              }`}
            ...
            <span className="ml-0.5 font-bold text-sm sm:text-xl" aria-label="multiplicado por 4">
              ×4
            </span>
          </div>
        </div>
        ...
        <fieldset className="border-0 p-0 mb-8">
          <legend className="sr-only">Digite o resultado da multiplicação</legend>
          <div className="w-full overflow-x-auto overflow-y-visible [-webkit-overflow-scrolling:touch]">
            ...
            <DigitInput value={inputs[5]} onChange={(v) => handleInputChange(5, v)} placeholder="5º" />
